locals {
  github_actions_role_arns = [
    "arn:aws:iam::${var.aws_account_id}:role/${var.github_actions_bootstrap_role_name}",
    "arn:aws:iam::${var.aws_account_id}:role/${var.github_actions_env_role_name}",
  ]
...
data "aws_iam_policy_document" "github_actions_bootstrap" {
  ...
  statement {
    ...
    resources = local.github_actions_role_arns
  }
